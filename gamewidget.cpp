    ...
    ui(new Ui::Widget)
{
    ...
    //设置步数
    connect(this,&GameWidget::StepChange,this,&GameWidget::setStep);
...
void GameWidget::initGame()
{
    ...
    if(file->open(QIODevice::ReadOnly))
    {
        ...
        QJsonValue step = json.value(QString("m_step"));
        QJsonValue time = json.value(QString("m_time"));
        ...
        //当前步数
        m_step = step.toVariant().toInt();
        this->ui->step_2->setText(QString::number(m_step));

        //当前时间
        m_time = time.toVariant().toInt();
        this->ui->time_2->setText(QString::number(m_time));
...
{    
    ...
    if(m_labelCount == 16)
    {
        ...
        if(!isWin){
            //没有可以合并的标签，显示失败
            QMessageBox::about(this, "信息", "可惜，再接再厉！");
            flag = true;
        }
    }
    else
...
void GameWidget::saveGame()
{
    ...
    if(file->open(QIODevice::WriteOnly))
    {
        ...
        json.insert("m_step",m_step);
        json.insert("m_time",--m_time);//因为在setTime函数里m_time提前加一，所以保存时要减掉
        d.setObject(json);
...
void GameWidget::setStep()
{
    this->ui->step_2->setText(QString::number(m_step));
}

void GameWidget::setTime()
{
    this->ui->time_2->setText(QString::number(m_time));
    ++m_time;
}
...
void GameWidget::on_bt_return_clicked(){
    saveGame();
    delete this;
